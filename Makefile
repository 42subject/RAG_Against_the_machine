...
STUDENT_RESULTS_PATH := $(SEARCH_RESULTS_DIRECTORY)/$(DATASET_NAME)
K ?= 5
...
run-search-dataset:
	$(PYTHON) -m src search_dataset \
		$(UNANSWERED_DATASET_PATH) $(SEARCH_RESULTS_DIRECTORY) --k=$(K)

run-answer-dataset:
	$(PYTHON) -m src answer_dataset \
		$(STUDENT_RESULTS_PATH) $(ANSWER_RESULTS_DIRECTORY)
